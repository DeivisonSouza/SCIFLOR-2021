...
data_sample <- data %>% 
  ...
  filter(Nome_Especie %in% c('Acapu', 'Tauari', 'Jatobá', 'Andiroba')) %>% 
...
```{r gg2, echo=T, eval=F}
ggplot(data = data_sample, mapping = aes(x = DAP, y = HC))
```
<br>
...
- A função .blue[geom_point()] foi usada para adicionar uma camada de pontos no gráfico. Tem-se um gráfico de dispersão.
]
]

.pull-right-4[
.font80[
```{r ref.label="gg3", out.width='80%', echo=F, eval=T}
```
]
]
