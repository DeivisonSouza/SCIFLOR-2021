...
&nbsp;&nbsp;&nbsp;&nbsp;[6.3 - Adicionando rótulos nas barras](#arb)
...
**Vamos experimentar 2 abordagens usando .green[geom_bar()] para construir esse gráfico...**
<br>
...
ggplot(data = data_sample2) +
  geom_bar(aes(x = Nome_Especie), stat = "count")
```
<br><br>
...
1. Por padrão, a camada .green[geom_bar()] usa .green[stat_count()] e faz a contagem de casos por categoria em "x".
...
ggplot(data = data_top) +
  geom_bar(aes(x = Nome_Especie, y = n), stat = "identity")
```
<br><br>
...
1. Nesta situação, usa-se .green[stat_identity()] na camada .green[geom_bar()], indicando que os dados devem ser usados como estão.
2. Neste caso, o argumento "y" precisa ser informado.
]
]
...
```{r}
ggplot(data = data_sample, aes(Nome_Especie, V)) +
...
ggplot(data = data_sample2, aes(Nome_Especie)) +
  geom_bar()
```
